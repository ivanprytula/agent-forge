...
EOF
    ...
    if grep -q '\.opencode/skills' "$opencode_config"; then
        sed -i 's|\.opencode/skills|../agent-forge/skills|g' "$opencode_config"
        echo "Fixed opencode.jsonc skills.paths"
    elif ! grep -q 'agent-forge/skills' "$opencode_config"; then
        sed -i 's|"paths": \[.*\]|"paths": ["../agent-forge/skills"]|g' "$opencode_config"
